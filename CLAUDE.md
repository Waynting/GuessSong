...
| `GET /api/quiz/[code]/hint?q=N[&refresh=1]` | A clip of question N's real track, via `getPreview`. Exists so the phone never names the answer in a request; `refresh=1` is the rotted-URL repair path, on its own tighter limit. |
| `GET /api/quiz/[code]/board` | The owner's results page: ranking, mean, per-question correct counts naming each real song. Host-token gated via the `x-host-token` header (403 `quiz_not_host`) because it is the answer key. |
...
**Taste Quiz is a loop surface with a game attached, and the key never leaves the server before an answer is in.** `lib/quiz.ts` (rules, pure, tested) and `lib/quiz-store.ts` (one hash per quiz, `quiz:v1:<CODE>`, mirroring `lib/room.ts`'s `hsetnx` claims) exist because the `share` arm had converted 0 of 50 and nothing link-shaped had been tried — `docs/decisions.md` D9. Six things hold it together:

- **No audio in a question.** Four titles, one real. A clip is a *hint*: `GET …/hint` on tap only, rationed by `hintAllowance` (one per five questions), refunded when the clip cannot be found or fails to play, a tiebreak on the board rather than a penalty. Playing a clip per question would multiply the hottest path in the app by the number of friends.
...
- **Decoys come from `lib/quiz-decoys.ts`, tiered, and never from the playlist.** Same artist, then any playlist artist, then same script and similar popularity, then same script, then anything. `songKey` strips qualifiers on both sides so a playlist "Hello - Live" excludes the pool's "Hello", and `displayTitle` strips them from the real option so " - Remastered" is not a tell. `scriptBucket` is derived from the strings on both sides; do not hand-tag the pool. A second Spotify call for decoys is deferred until `quiz_verdict:*` piles at `soulmate`.
...
`/guides`, `/privacy`, `/terms` and `/contact` must stay out of `app/robots.ts`'s disallow list. That list is for ephemeral room and quiz codes and the counting redirect; a content page landing in it would be invisible to exactly the crawler it was written for. `tests/site-policy.test.ts` asserts this.
