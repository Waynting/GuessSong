...
**`buzzerWorkerUrl()` in `lib/buzzer-client.ts` is the only reader of that variable, and it trims.** `isBuzzerConfigured` (the toggle on `/`), `httpOrigin` (the host's one `POST`) and `socketUrl` (the phone's socket) all go through it, so the three agree on what "no Worker" means: a blank or whitespace-only value hides the toggle *and* makes `socketUrl` return null, where a second reading would hand `new WebSocket()` a bare `/rooms/<code>/ws` (a relative URL, the same crash) and send the host's POST to the page's own `/rooms`. The trim is not tidiness: a leading space pasted into the dashboard field misses the `://` anchor of both folds while the URL parser strips it, so a new browser connects, the host's POST succeeds, nothing on a developer's machine notices, and the old constructor throws on it, the crash back from one character. `httpOrigin` is the same fold in the other direction (`ws(s)://` into `http(s)://`, anchored, case-insensitive), and the two must stay mirrors so either spelling names one Worker from both sides. `tests/buzzer.test.ts` pins all of it against the production value.

**`mintPlayerId()` must keep its fallback chain, every storage call on the buzzer surfaces goes through `lib/host-session.ts` (`readStored` / `writeStored` / `removeStored`, or `withStorage` for a multi-step read), and `new WebSocket` stays inside its `try`.** `getPersistentPlayerId` used to be `localStorage.getItem` and `crypto.randomUUID()` back to back, unguarded, inside the `useMemo` that runs on `useBuzzerSocket`'s first render — so on Safari/iOS before 15.4 or Chrome before 92 (no `randomUUID`), and on any browser that throws on the storage access, the setup page became the crash screen the moment Buzzer Mode was switched on, `/game` did the same in a buzzer game, and `/buzz/[code]` never showed the form. Reported as "I can't start the game, it always gives an error and asks me to restart"; never reproduces on a developer's machine. The fallback is `getRandomValues` and then `Math.random`, both shaped as a v4 UUID because the Worker is handed it as the identity it dedupes on, and a stored value that is not that shape is minted over. The socket constructor is the third throw on the same effect path — a `ws://` from an https page is refused synchronously as mixed content, so `socketUrl` upgrades it to `wss://` there and the `try` reports anything else as `unreachable`, one of the page's own client codes (`BUZZER_CLIENT_ERROR_CODES`, beside `not_configured` for an unset Worker URL and `no_answer` for a room that never answered three times running), so the sentence names the thing to fix. A browser that keeps no id presents a new one per page load, which is why `worker/src/buzzer-room.ts` hands a seat whose owner has no socket to a join under the same name (`takeSeat`) rather than refusing it — under three rules that must stay: a connected owner is never displaced by a name, the id that opened a seat (`adoptedFrom`) reclaims it from any taker, and the host's seat (`hostPlayerId`) yields to the token alone and to it always, because a guest who took the name "Host" while the host's socket was closed for the `/` → `/game` navigation locked the token holder out of their own room. `tests/buzzer.test.ts` pins the chain, the degradation and the guard by reading the source; `worker/test/buzzer-room.test.ts` pins adoption; `docs/operations.md` §5 has the repro.
...
The buzzer Worker keeps its own wire codes (`BuzzerErrorCode` in `lib/buzzer-protocol.ts`, which is shared verbatim with the Worker and must stay dependency-free). `BUZZER_ERROR_CODES` maps them onto app codes — that mapping is the only thing connecting the two, so a new wire code needs an entry there. Two siblings sit beside it and must not be folded into it: `BUZZER_CLIENT_ERROR_CODES` is the page's own vocabulary for a socket it never opened or that never answered (`not_configured`, `unreachable`, `no_answer`), and `BUZZER_HOST_ERROR_CODES` is the host's reading of a refusal whose player sentence is wrong on the host's screen ("ask the host for a new one"; a player is told to shout, the host what to fix). `buzzerErrorMessage(error, locale, audience)` is the one render path for all three, and `tests/error-messages.test.ts` pins that nothing the host's chair renders says to ask the host.
...
Five things in that path are easy to undo by accident:
...
- **`share` is a retired surface, and the only one allowed to show `followed` with no `shown`.** The result card's QR read 0 of 94 over eleven weeks and came off on 2026-09-22 (`lib/result-image.ts` prints the address as text); the name stays in `LOOP_SURFACES` so a card saved before then still redirects and counts. Two things keep it off: `loopQrDataUrl` takes its surface as a required argument — the old `= "share"` default is exactly how a card would grow the code back — and nothing calls `reportLoopImpression("share")`. `docs/viral-loop.md` §6.
- **`loadPlaylist` counts the four permanent refusals (`playlist_refused:<code>`) on the way out, and that wrapper is the only place that sees every caller.** An editorial playlist is refused before the cache is read, so it was in no cache statistic at all; `playlist_empty` is raised by the routes, not the cache, so it is counted from the empty result rather than the throw. Moving the count into `app/api/playlist/route.ts` loses the Mixed room's and the quiz's refusals; counting a throttling code there double-counts the budget block.
