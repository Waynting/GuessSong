...
| `POST /api/quiz/[code]/check` | `{q, pick}` → `{answer, correct}`, the verdict on one question the moment it is answered. Hands over question N's answer only against a pick for N, and records nothing — the board is still written once, by `answer`. `q === 0` counts `quiz:started`. |
