  ...
  return (
    <div className="rounded-2xl bg-[#1a1a1a] p-4 text-white">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-widest text-[#888]">ROOM CODE</p>
          <p className="text-4xl font-bold tracking-[0.2em] text-[#1DB954]">{roomCode}</p>
          <p className="mt-1 text-xs text-[#888]">
            {connected
              ? `${players.filter((p) => p.connected).length} of ${players.length} phones connected`
              : "Connecting…"}
          </p>
        </div>
        {qr && (
          // eslint-disable-next-line @next/next/no-img-element -- data: URI generated client-side, nothing for next/image to optimise
          <img src={qr} alt={`QR code to join room ${roomCode}`} className="h-28 w-28 rounded bg-white p-1" />
        )}
      </div>
      ...
      <div className="mt-4 min-h-[4.5rem] rounded-xl bg-[#111] p-3">
        {current ? (
          ...
          <p className="text-sm text-[#666]">
            {snapshot?.phase === "open" ? "Open — nobody has buzzed yet" : "Waiting for the next clip"}
          </p>
        )}
      </div>
