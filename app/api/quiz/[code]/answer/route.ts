...
import { recordQuizStage, recordQuizVerdict } from "@/lib/loop-stats";
...
export async function POST(
  ...
  try {
    ...
    // Two independent, fail-soft counters: one round trip, not two in series
    // on the response the taker is waiting for.
    await Promise.all([recordQuizStage("completed"), recordQuizVerdict(result.verdict)]);
